function quix_compiler(function_name, destination_folder, do_zip)
    % QUIX_COMPILER Compiles a MATLAB function to Python wheel for Quix deployment
    %
    % This function compiles a MATLAB/Simulink wrapper function into a Python
    % package that can be installed and used with the MATLAB Runtime.
    %
    % Inputs:
    %   function_name       - Name of the MATLAB function file (e.g., 'simulink_wrapper')
    %   destination_folder  - Destination folder for the compiled output
    %   do_zip              - (Optional) Boolean flag: zip the output folder? Default = true
    %
    % Example:
    %   quix_compiler('simulink_wrapper', 'build_output')
    %   quix_compiler('simulink_wrapper', 'build_output', false)
